
module.exports = {
    RESULTS:{
        SUCCESS:"SUCCESS",
        FAILURE:"FAILURE"    
    },
    DB:{
        host     : 'localhost',
        user     : 'root',
        password : 'root',
        database : 'db_gdinsp'
    },
    DB_QUERIES:{
        USER:{
            GETUSERBYID: "SELECT * FROM USER WHERE ID=?",
            GETUSERBYLOGINID: "SELECT * FROM USER WHERE LOGIN_ID=?",
            GETALLUSERS: "SELECT * FROM USER",
            INSERTUSER: "INSERT INTO USER (NAME,TITLE,DESIGNATION,ROLE,LOGIN_ID,PASSWORD,UPDATED_BY,UPDATE_TS,STATE) VALUES (?,?,?,?,?,?,?,?,?)",
            UPDATEUSERBYID: "UPDATE USER SET NAME=?, TITLE=?, DESIGNATION=?, ROLE=?, LOGIN_ID=?,PASSWORD=?,UPDATED_BY=?,UPDATE_TS=?,STATE=? WHERE ID=?",
            DELUSERBYID: "UPDATE USER SET STATE=0 WHERE ID=?"
        },
        VILLAGE:{
            GETVILLAGEBYID: "SELECT * FROM VILLAGE WHERE ID=?",
	        GETVILLAGEBYDISTRICT: "SELECT * FROM VILLAGE WHERE DISTRICT=?",
            GETALLVILLAGES: "SELECT * FROM VILLAGE",
            INSERTVILLAGE: "INSERT INTO VILLAGE (NAME,BLOCK,TEHSIL,DISTRICT,UPDATED_BY,UPDATE_TS,STATE) VALUES (?,?,?,?,?,?,?)",
            UPDATEVILLAGEBYID: "UPDATE VILLAGE SET NAME=?, BLOCK=?,TEHSIL=?,DISTRICT=?,UPDATED_BY=?,UPDATE_TS=?,STATE=? WHERE ID=?",
            DELVILLAGEBYID: "UPDATE VILLAGE SET STATE=0 WHERE ID=?"
        },
        VILLAGE_GROUP:{
            GETVILLAGEGROUPBYID: "SELECT * FROM VILLAGE_GROUP WHERE ID=?",
            GETALLVILLAGEGROUPS: "SELECT * FROM VILLAGE_GROUP",
            INSERTVILLAGEGROUP: "INSERT INTO VILLAGE_GROUP (NAME,OFFICER_ID,UPDATED_BY,UPDATE_TS,STATE) VALUES (?,?,?,?,?)",
            UPDATEVILLAGEGROUPBYID: "UPDATE VILLAGE_GROUP SET NAME=?, OFFICER_ID=?, UPDATED_BY=?,UPDATE_TS=?,STATE=? WHERE ID=?",
            DELVILLAGEGROUPBYID: "UPDATE VILLAGE_GROUP SET STATE=0 WHERE ID=?",

            GETVGVILLAGESBYID: "SELECT * FROM VILLAGE_GROUP_MAP WHERE ID=?",
            INSERTVGVILLAGE: "INSERT INTO VILLAGE_GROUP_MAP (NAME,VILLAGE_GROUP_ID,VILLAGE_ID,UPDATED_BY,UPDATE_TS,STATE) VALUES (?,?,?,?,?,?)",
            DELVGVILLAGEBYID: "DELETE VILLAGE_GROUP_MAP WHERE VILLAGE_ID=?",
            DELVGVILLAGESBYVGID: "DELETE VILLAGE_GROUP_MAP WHERE VILLAGE_GROUP_ID=?"
        },
        INSTITUTION:{
            GETINSTITUTIONBYID: "SELECT * FROM INSTITUTION WHERE ID=?",
            GETALLINSTITUTIONS: "SELECT * FROM INSTITUTION",
            INSERTINSTITUTION: "INSERT INTO INSTITUTION (NAME,VILLAGE_ID,TYPE,HEAD_NAME,ADDRESS,CONTACT,UPDATED_BY,UPDATE_TS,STATE) VALUES (?,?,?,?,?,?,?,?,?)",
            UPDATEINSTITUTIONBYID: "UPDATE INSTITUTION SET NAME=?, VILLAGE_ID=?,TYPE=?,HEAD_NAME=?,ADDRESS=?,CONTACT=?,UPDATED_BY=?,UPDATE_TS=?,STATE=? WHERE ID=?",
            DELINSTITUTIONBYID: "UPDATE INSTITUTION SET STATE=0 WHERE ID=?"
        },
        CHECKLIST:{
            GETCHECKLISTBYID: "SELECT * FROM CHECKLIST WHERE ID=?",
            GETALLCHECKLISTS: "SELECT * FROM CHECKLIST",
            INSERTCHECKLIST: "INSERT INTO CHECKLIST (NAME,TITLE, DESIGNATION, ROLE, LOGIN_ID, PASSWORD, UPDATED_BY, UPDATE_TS, STATE) VALUES (?,?,?,?,?,?,?,?)",
            UPDATECHECKLISTBYID: "UPDATE CHECKLIST SET NAME=?, TITLE=?, DESIGNATION=?, ROLE=?, LOGIN_ID=?,PASSWORD=?,UPDATED_BY=?,UPDATE_TS=?,STATE=? WHERE ID=?",
            DELCHECKLISTBYID: "UPDATE CHECKLIST SET STATE=0 WHERE ID=?"
        },
        CHECKPOINT:{
            GETCHECKPOINTBYID: "SELECT * FROM CHECKPOINT WHERE ID=?",
            GETALLCHECKPOINTS: ",SELECT * FROM CHECKPOINT",
            INSERTCHECKPOINT: "INSERT INTO CHECKPOINT (NAME,DESC,INST_TYPE,UPDATED_BY,UPDATE_TS,STATE) VALUES (?,?,?,?,?,?)",
            UPDATECHECKPOINTBYID: "UPDATE CHECKPOINT SET NAME=?, DESC=?, INST_TYPE=?,UPDATED_BY=?,UPDATE_TS=?,STATE=? WHERE ID=?",
            DELCHECKPOINTBYID: "DELETE CHECKPOINT WHERE ID=?",
            DELCHECKPOINTBYCHECKLISTID: "DELETE CHECKPOINT WHERE CHECKLIST_ID=?"
        },
        CONFIG:{
            GETCONFIGBYKEYVAL:"SELECT * FROM CONFIG WHERE CONFIG_KEY=? and CONFIG_VALUE=?"
        }
    },
    DB_QUERY_TYPES:{
        INSERT:"I", UPDATE:"U",LIST:"L",DELETE:"D"
    },
    DBCONFIG:{CLIENTKEY:"CLIENTKEY"},
    ERRORS:{
        REQ_PARAM_MISSING:404
    }

}